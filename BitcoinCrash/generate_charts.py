...
def generate_charts(df):
    ...
    # Highlight the crash area
    crash_start = datetime(2026, 2, 4)
    crash_end = datetime(2026, 2, 7)
    plt.axvspan(crash_start, crash_end, color='red', alpha=0.1, label='The Unwind')
    
    plt.title('Bitcoin vs. Software Equities: The New Correlation (Indexed to 100)', fontsize=14, fontweight='bold')
    ...
    plt.title('Bitcoin vs. Gold: The Narrative Divergence', fontsize=14, fontweight='bold')
    plt.ylabel('Indexed Price')
    plt.legend()
    plt.tight_layout()
    ...
    plt.annotate('Mechanical Crash\n(Feb 5)', 
                 ...
                 xytext=(datetime(2026, 2, 1), btc_detail.loc['2026-02-05'] * 0.9),
